...
export default function ProjectDetailScreen() {
  ...
  const updatePatternSnippet = useProjectsStore((state) => state.updatePatternSnippet);
  ...
  return (
    <Screen>
      <ScrollView contentContainerStyle={styles.content}>
        <Card
          title={project.name}
          subtitle={project.patternName ?? 'Custom plan'}
          highlight
          style={styles.section}>
          <Text style={{ color: theme.colors.textSecondary }}>
            {project.patternSourceType === 'external' && project.sourceUrl
              ? 'External link'
              : project.patternSourceType === 'built-in'
                ? 'Built-in pattern'
                : 'My pattern'}
          </Text>
        ...
        <Card title="Pattern info" style={styles.section}>
          {project.sourceUrl ? (
            <TouchableOpacity onPress={() => Linking.openURL(project.sourceUrl!)}>
              <Text style={{ color: theme.colors.accent }}>Open reference link</Text>
            </TouchableOpacity>
          ) : null}
          <Text style={[styles.bodyText, { color: theme.colors.textSecondary }]}>
            Yarn weight: {project.yarnWeight ?? 'n/a'}
          </Text>
          <Text style={[styles.bodyText, { color: theme.colors.textSecondary }]}>
            Hook size: {project.hookSizeMm ? `${project.hookSizeMm} mm` : 'n/a'}
          </Text>
        </Card>

        <Card title="Yarn & stash" subtitle="Reserve skeins and keep totals accurate" style={styles.section}>
        ...
        <Card title="Pattern snippet" style={styles.section}>
          <TextInput
            multiline
            numberOfLines={6}
            defaultValue={project.patternSnippet}
            onEndEditing={(event) => updatePatternSnippet(project.id, event.nativeEvent.text)}
        ...
        <Card title="Future tools" subtitle="Phase 2 placeholders" style={styles.section}>
          <Text style={{ color: theme.colors.textSecondary, marginBottom: 8 }}>
            We’re carving out space for upcoming helpers. These stubs keep navigation ready without
            overwhelming the current experience.
          </Text>
          <Text style={{ color: theme.colors.textSecondary }}>• Diagram & drawing canvas</Text>
          <Text style={{ color: theme.colors.textSecondary }}>• Voice & video notes</Text>
          <Text style={{ color: theme.colors.textSecondary }}>• Moodboard & inspiration pins</Text>
          <Text style={{ color: theme.colors.textSecondary }}>• Advanced linked counters</Text>
          <Text style={{ color: theme.colors.textSecondary }}>
            • Sync + social sharing (in progress)
          </Text>
        </Card>
      </ScrollView>
